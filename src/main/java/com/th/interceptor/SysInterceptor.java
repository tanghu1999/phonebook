...
import com.th.entity.CheckResult;
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        if (handler instanceof HandlerMethod){
            ...
            if (StringUtils.isEmpty(authHeader)) {
                logger.info("验证失败");
                this.print(response, R.error(SystemConstant.JWT_ERRCODE_NULL,"签名验证不存在"));
                ...
                if (checkResult.isSuccess()) {
                    ...
                    switch (checkResult.getErrCode()) {
                        ...
                        case SystemConstant.JWT_ERRCODE_FAIL:
                            logger.info("签名验证不通过");
                            this.print(response,R.error(checkResult.getErrCode(),"签名验证不通过"));
                            break;
                        // 签名过期，返回过期提示码
                        case SystemConstant.JWT_ERRCODE_EXPIRE:
                            logger.info("签名过期");
                            this.print(response,R.error(checkResult.getErrCode(),"签名过期"));
                            break;
                        default:
                            break;
                    }
